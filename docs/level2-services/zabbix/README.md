<div align="center">

# 🚧 Dokumentacja w Trakcie Tworzenia 🚧

![Status](https://img.shields.io/badge/STATUS-WORK_IN_PROGRESS-orange?style=for-the-badge)

### 👨‍💻 Spokojnie, to tylko przerwa techniczna.
Systemy działają, ale dokumentacja jeszcze się pisze.
Muszę przekopać się przez logi i screenshoty, żeby przygotować tu konkretny materiał. A tak naprawdę brakuje mi czasu przez CCNA...

<img src="https://media.giphy.com/media/13HgwGsXF0aiGY/giphy.gif" width="400">

<br>
<br>

---

### ⏳ Co tu się znajdzie?
* Pełna konfiguracja (pliki config).
* Rozwiązane problemy (Troubleshooting).

<br>

[⬅️ Powrót do Mapy Głównej](../../README.md)

</div>